				...
				<tr >
				   <td width="10%" class="text-right" style="vertical-align: middle;font-weight:bold;">
			  		...
			  		<td width="14%">
				    		...
				    		<option value="lx"  <c:if test='${bx.bizType==lx}'>selected</c:if> >留学</option>
				    		<option value="px"  <c:if test="${bx.bizType==px}">selected</c:if>>培训</option>
				    		<option value="ym"  <c:if test="${bx.bizType==ym}">selected</c:if>>移民</option>
				    		<option value="xly" <c:if test="${bx.bizType==xly}">selected</c:if>>夏令营</option>
				    		<option value="zh"  <c:if test="${bx.bizType==zh}">selected</c:if>>综合</option>
				    		<option value="qt"  <c:if test="${bx.bizType==qt}">selected</c:if>>其他</option>
				    	</select>
			    	</td>
